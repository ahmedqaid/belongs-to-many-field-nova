...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(1)("30bbe8cf", content, false, {});
// Hot Module Replacement
if(false) {
 // When the styles change, update the <style> tags
 if(!content.locals) {
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(1)("5590b347", content, false, {});
// Hot Module Replacement
if(false) {
 // When the styles change, update the <style> tags
 if(!content.locals) {
  ...
  methods: {
    ...
    registerDependencyWatchers: function registerDependencyWatchers(root) {
      ...
      root.$children.forEach(function (component) {
        if (component.field) {
          // logging only
          console.log("Component: " + component.field.name, component.field.component, component, component.field);
        }
        if (_this.componentIsDependency(component)) {
          console.log("component.selectedResourceId", component.selectedResourceId);
          if (component.selectedResourceId !== undefined) {
            var attribute = _this.findWatchableComponentAttribute(component);
            component.$watch(attribute, _this.dependencyWatcher, {
              immediate: true
            });
            _this.dependencyWatcher(component.selectedResourceId);
          } else if (component.field && component.field.component === "BelongsToManyField") {
            _this.parentType = "belongs-to-many";
            component.$watch("value", _this.dependencyWatcherBelongsToMany, {
              immediate: true
            });
          }
        }
    ...
    findWatchableComponentAttribute: function findWatchableComponentAttribute(component) {
      console.log("findWatchableComponentAttribute", component.field.component, component, component.field);
    ...
    dependencyWatcherBelongsToMany: function dependencyWatcherBelongsToMany(value) {
      console.log("dependencyWatcherBelongsToMany", value);
      if (value === this.dependsOnValue) {
        return;
      }
      this.dependsOnValue = value.map(function (item) {
        return item.id;
      }).join(",");
      this.fetchOptions();
    },
    ...
    fetchOptions: function fetchOptions() {
      ...
      if (this.isDependant) {
        console.log("fetchOptions - isDependant - dependsOnValue: ", this.dependsOnValue);
        if (this.dependsOnValue) {
          console.log("fetchOptions - has dependsOnValue");
          this.loading = true;
          Nova.request(baseUrl + this.resourceName + "/" + (this.parentType === "belongs-to-many" ? "combined-options/" : "options/") + this.field.attribute + "/" + this.optionsLabel + "/" + this.dependsOnValue + "/" + this.field.dependsOnKey + "/" + this.field.dependsOnOverride).then(function (data) {
            console.log("fetchOptions - data", data.data);

            // Map data to group
            var result = data.data.reduce(function (acc, item) {
